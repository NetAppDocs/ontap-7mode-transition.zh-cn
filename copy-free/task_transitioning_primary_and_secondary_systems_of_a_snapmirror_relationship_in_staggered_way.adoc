...
.. [[_step_2b]] 使用  `job schedule cron create` 命令创建与为 7-Mode SnapMirror 关系配置的计划相匹配的作业计划。
.. [[_step_2c]] 使用  `snapmirror create` 命令在 7-Mode 主卷和 ONTAP 辅助卷之间创建 TDP 类型的 SnapMirror 关系。
...
.. 使用 `snapmirror delete`命令删除在<<_step_2c,步骤 2c>>中创建的 7-Mode 主卷和 ONTAP 辅助卷之间的 TDP SnapMirror 关系。
.. 删除在 <<_step_2b,步骤 2b>> 中创建的 cron 作业计划：+
