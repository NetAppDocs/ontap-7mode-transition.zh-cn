...
如果主机未启动 SAN，并且您需要将服务运行到非转换 LUN 或不是由 NetApp 制造的阵列 LUN，则可以选择不将其关闭。但是，如果不关闭它，您可能会遇到全路径关闭 (APD) 情况。为了避免 APD 情况，可以屏蔽在 7-Mode 中运行的 Data ONTAP LUN。有关详细信息，请参见 link:https://knowledge.broadcom.com/external/article/304594["VMware 知识库 ID 304594"]。
